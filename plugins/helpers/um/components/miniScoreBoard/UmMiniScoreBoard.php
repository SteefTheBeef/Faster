...
class UmMiniScoreBoard {
    private static function getHeight(UmMiniScoreBoardRenderContext $ctx) {
        $numPlayers = count($ctx->umState->grandFinalRankings);
        return $numPlayers * 1.7 * 1.5 + 7;
    }
    ...
    public static function buildPanelXml(UmMiniScoreBoardRenderContext $ctx) {
        ...
        $height = self::getHeight($ctx);

        return XmlTag::frame($geometry->mainFrameX, $geometry->mainFrameY, 1,
            "<quad sizen=\"22 {$height}\" posn='0 0' style='BgsPlayerCard' substyle='BgCardSystem'/>"
            ...
            . XmlTag::label(3.1, $y, 20, 2, "\$l[https://umts.vercel.app/um4/grand-final/leaderboard]\$0f0\$oUM4 Grand-Final Score\$l")
    ...
    private static function buildTable(UmMiniScoreBoardRenderContext $ctx, $y) {
        ...
        $players = $ctx->umState->grandFinalRankings;
