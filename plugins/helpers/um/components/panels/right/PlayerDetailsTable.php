...
class PlayerDetailsTable {
    static function build(UmPanelRenderContext $ctx, $selectedPlayer) {
        ...
        foreach($selectedPlayer['PerMap'] as $envi => $values) {
            ...
            $i = $i + 1;
            $rows[$i]['envi'] = $envi;
            $rows[$i]['type'] = 'Best Lap';
            $rows[$i]['rank'] = $values['BestLapRank'];
            $rows[$i]['time'] = $values['BestLapTime'];
            $rows[$i]['score'] = $values['BestLapScore'];
            $rows[$i]['rankInt'] = $values['BestLapRank'];
            $i = $i + 1;
        }
