...
// Init : (plugin init)
//--------------------------------------------------------------
function um_liveinfosInit($event){
	...
	if($_mldebug>4) console("um_liveinfos.Event[$event]");
	...
	//manialinksGetHudPartControl('um_liveinfos','checkpoint_list');
}


function um_liveinfosPlayerConnect($event,$login){
  ...
  if(!is_string($login))
	...
	if($_players[$login]['ML']['Show.position'] && $_players[$login]['Status2']<2)
		um_liveinfosUpdatePlayerPositionXml($login,'show');
	...
	if($_players[$login]['ML']['Show.topgaps'] && $_players[$login]['Status2']<2)
		um_liveinfosUpdatePlayerTopGapsXml($login,'show');
	...
	if($_players[$login]['ML']['Show.liveplayers'] && $_players[$login]['Status2']<1)
		um_liveinfosUpdatePlayerLivePlayersXml($login,'show');
}


function um_liveinfosPlayerMenuBuild($event,$login){
...
function um_liveinfosPlayerMenuAction($event,$login,$action,$state){
	global $_mldebug,$_players;
	//if($_mldebug>6) console("um_liveinfos.Event[$event]('$login',$action,$state)");
	if(!isset($_players[$login]['Relayed']) || $_players[$login]['Relayed'])
		return;
	...
	if($action=='menu.hud.position'){
		...
		if($state && $_players[$login]['ML']['ShowML'] && $_players[$login]['ML']['Show.live'] && $_players[$login]['Status2']<2)
			...
			um_liveinfosUpdatePlayerPositionXml($login,'hide');
		if($state)
		...
		if($state && $_players[$login]['ML']['ShowML'] && $_players[$login]['ML']['Show.live'] && $_players[$login]['Status2']<2)
			...
			um_liveinfosUpdatePlayerTopGapsXml($login,'hide');
		if($state)
		...
		if($_players[$login]['ML']['Show.liveplayers']!=$state)
			um_liveinfosUpdatePlayerLivePlayersXml($login,'hide');
...
function um_liveinfosPlayerShowML($event,$login,$ShowML){
	global $_mldebug,$_players;
	if(!isset($_players[$login]['Relayed']) || $_players[$login]['Relayed'])
		return;
...
function um_liveinfosPlayerPositionChange($event,$login,$changes){
	global $_mldebug,$_players;
	if($login===true){
		//if($_mldebug>5) console("um_liveinfos.Event[$event](true,$changes)");

		// some change in positions
		um_liveinfosUpdatePlayerPositionXml(true,'refresh');
		//um_liveinfosUpdatePlayerLivePlayersXml(true,'refresh');

	}elseif(is_string($login)){
		//if($_mldebug>5) console("um_liveinfos.Event[$event]($login,$changes)");
		if(!isset($_players[$login]['Relayed']) || $_players[$login]['Relayed'])
			return;
		...
		if($_players[$login]['ML']['ShowML'] && $_players[$login]['ML']['Show.liveplayers']>0){
			um_liveinfosUpdatePlayerLivePlayersXml($login,$changes);
		}
...
function um_liveinfosPlayerTeamChange($event,$login,$teamid){
	um_liveinfosUpdatePlayerLivePlayersXml(true,'refresh');
}


function um_liveinfosEverysecond($event,$sec){
	...
	if($_GameInfos['GameMode'] == TEAM){ // team
		if($live_team0_score!=$_teams[0]['Score'] || $live_team1_score!=$_teams[1]['Score']){
			...
			um_liveinfosUpdatePlayerLivePlayersXml(true,128);
		}
	}
}


function um_liveinfosPlayerCheckpoint($event,$login,$time,$lapnum,$checkpt){
	global $_mldebug,$_players,$_checkpoints_hide,$_players_playing;
	if($_mldebug>6) console("um_liveinfos.Event[$event]('$login',$time,$lapnum,$checkpt)");
	...
	foreach($_players as $speclogin => &$pl){
		if($pl['Status']==1 && $pl['FinalTime']<=0 && ($pl['CurrentTargetId']==$pid || $_players_playing==1) && 
			...
			if($pl['ML']['Show.topgaps'])
				um_liveinfosUpdatePlayerTopGapsXml(''.$speclogin,'show',$login);
		}
	}
}


function um_liveinfosPlayerSpecChange($event,$login,$isspec,$specstatus,$oldspecstatus){
	global $_mldebug,$_players;
	//if($_mldebug>0) console("um_liveinfos.Event[$event]('$login',".($isspec?'true':'false').",$specstatus,$oldspecstatus)");
	if(!isset($_players[$login]['Relayed']) || $_players[$login]['Relayed'])
		return;
...
function um_liveinfosPlayerStatus2Change($event,$login,$status2,$oldstatus2){
	global $_debug,$_players;
	//console("um_liveinfos.Event[$event]('$login',$status2)");
...
function um_liveinfosUpdatePlayerCPGapsXml($login,$action='show',$speclogin=''){
	global $_mldebug,$_players,$_StatusCode,$_BestPlayersChecks,$_GameInfos,$_BestChecks,$_IdealChecks,$_BestChecksName,$_ChallengeInfo;
	// if the players disabled manialinks then do nothing
	if(!$_players[$login]['ML']['ShowML'])
		return;
	if($_mldebug>3) console("um_liveinfosUpdatePlayerCPGapsXml('$login',$action) - ".$_players[$login]['Status']);

	if($action=='remove'){
		// remove manialink
		manialinksRemove($login,'um_liveinfos.cp');
		return;
	}elseif($action=='hide'){
		// hide manialink
	...
	$filename = "fastlog/6laps/".$_ChallengeInfo['UId']."_".$login.".txt";

	if (!file_exists($filename)) {
		return;
	}

	$cpFile = fopen("fastlog/6laps/".$_ChallengeInfo['UId']."_".$login.".txt", "r");
	if (!$cpFile || !isset($_players[$login]["Checkpoints"]) || count($_players[$login]["Checkpoints"]) === 0) {
		return;
	}
	...
	while(!feof($cpFile)) {
		array_push($fileArray,fgets($cpFile));
	}
	...
	$cpCount = count($_players[$login]["Checkpoints"]);
	$arr = $_players[$login]["Checkpoints"];
	array_shift($arr);
	//array_shift($arr);
	$currentCp = array_pop($arr);
	$fileCp = $checkpointArray[$cpCount - 2];

	if($_players[$login]['FinalTime']>=0){
		// final time
		$msg .= ' $z$s$w'.MwTimeToString($_players[$login]['FinalTime']).'  ';
		console("$_players[$login]['FinalTime']>=0");
	}else{
		// check number
		...
		if(isset($_BestPlayersChecks[$login][$key]) && count($_BestPlayersChecks[$login][$key])>0){
			...
			console("yayyay");
			if($diff>0)
				$msg .= '$f00.  $z$s$w$f30'.MwDiffTimeToString($diff);
				//$msg .= 'Hej';
			elseif($diff<0 || $_players[$login]['FinalTime']<=0)
				$msg .= '$f00.  $z$s$w$00C'.MwDiffTimeToString($diff);
			else
				$msg .= '$f00.  $z$s$i$00CRecord';
	...
	if($msg!=''){
		...
		//console("um_liveinfosUpdatePlayerCPGapsXml - $showlogin - $xml");
...
// action can be 'show', 'hide', 'remove'
//--------------------------------------------------------------
function um_liveinfosUpdatePlayerTopGapsXml($login,$action='show',$speclogin=''){
	...
	if($action=='remove'){
		// remove manialink
		manialinksRemove($login,'um_liveinfos.tops');
		return;
	}elseif($action=='hide'){
		// hide manialink
		manialinksHide($login,'um_liveinfos.tops');
		return;
	}
	// show/refresh
	if(!$_players[$login]['ML']['Show.live'] || !$_players[$login]['ML']['Show.topgaps'] || 
		 ($_players[$login]['IsSpectator'] && $speclogin=='')){
		// none to show and opened : hide it
		if(manialinksIsOpened($login,'um_liveinfos.tops'))
			manialinksHide($login,'um_liveinfos.tops');
		return;
	}
	...
	if($msg!=''){
		...
		//console("um_liveinfosUpdatePlayerTopGapsXml - $showlogin - $xml");
		manialinksShow($showlogin,'um_liveinfos.tops',$xml);
	}
}
...
// action can be 'show', 'hide', 'remove'
//--------------------------------------------------------------
function um_liveinfosUpdatePlayerPositionXml($login,$action='show'){
	...
	if($login===true){
		foreach($_players as $login => &$pl){
			if(isset($pl['ML']) && $pl['Status2']<2 && $pl['ML']['ShowML'] && $pl['ML']['Show.live'])
				um_liveinfosUpdatePlayerPositionXml($login,'refresh');
		}
		return;
	}
	// if the players disabled manialinks then do nothing
	...
	if($action=='remove'){
		// remove manialink
		manialinksRemove($login,'um_liveinfos.pos');
		return;
	}elseif($action=='hide'){
		// hide manialink
		manialinksHide($login,'um_liveinfos.pos');
		return;
	}
	// show/refresh
	if(!$_players[$login]['ML']['Show.live'] || !$_players[$login]['ML']['Show.position'] ||
		...
		// none to show and opened : hide it
		if(manialinksIsOpened($login,'um_liveinfos.pos'))
			manialinksHide($login,'um_liveinfos.pos');
		return;
	}
	...
	//console("um_liveinfosUpdatePlayerPositionXml - $login - $xml");
	manialinksShow($login,'um_liveinfos.pos',$xml);
}
...
function um_liveinfosUpdatePlayerLivePlayersXml($login,$action='show'){
	...
	if($login===true){
		foreach($_players as $login => &$pl){
			if(isset($pl['ML']) && $pl['Status2']<1 && $pl['ML']['ShowML'] && $pl['ML']['Show.live'])
				um_liveinfosUpdatePlayerLivePlayersXml($login,$action);
		}
		return;
	}
	// if the players disabled manialinks then do nothing
	if(!$_players[$login]['ML']['ShowML'])
		return;
	//if($_mldebug>8) console("um_liveinfosUpdatePlayerLivePlayersXml($login,$action)");

	if($action=='remove'){
		// remove manialink
		manialinksRemove($login,'um_liveinfos.players.1');
		manialinksRemove($login,'um_liveinfos.players.2');
		manialinksRemove($login,'um_liveinfos.players.3');
		manialinksRemove($login,'um_liveinfos.players.4');
		manialinksRemove($login,'um_liveinfos.players.5');
		manialinksRemove($login,'um_liveinfos.players.6');
		manialinksRemove($login,'um_liveinfos.players.bg');
		return;
	}elseif($action=='hide'){
		// hide manialink
		manialinksHide($login,'um_liveinfos.players.1');
		manialinksHide($login,'um_liveinfos.players.2');
		manialinksHide($login,'um_liveinfos.players.3');
		manialinksHide($login,'um_liveinfos.players.4');
		manialinksHide($login,'um_liveinfos.players.5');
		manialinksHide($login,'um_liveinfos.players.6');
		manialinksHide($login,'um_liveinfos.players.bg');
		return;
	}
	// show/refresh
	if(!$_players[$login]['ML']['Show.live'] || $_players[$login]['ML']['Show.liveplayers'] <= 0 ||
		...
		// none to show and opened : hide it
		if(manialinksIsOpened($login,'um_liveinfos.players.bg')){
			manialinksHide($login,'um_liveinfos.players.1');
			manialinksHide($login,'um_liveinfos.players.2');
			manialinksHide($login,'um_liveinfos.players.3');
			manialinksHide($login,'um_liveinfos.players.4');
	...
	if(!manialinksIsOpened($login,'um_liveinfos.players.bg')){
		if($lip<2)
			manialinksShow($login,'um_liveinfos.players.bg',$_live_players_bg_xml);
		else
			manialinksShow($login,'um_liveinfos.players.bg',$_live_players_bg2_xml);
	}
	...
	if($action & (1+128)){
		...
		manialinksShow($login,'um_liveinfos.players.4',$xml);
	}
		...
		*/
		...
		if($ppos>2 && $position['FirstLogin']!='' && isset($_players[$position['Prev2Login']])){
			if($action & 16){
				...
				manialinksShow($login,'um_liveinfos.players.1',$xml);
			}
		}elseif(manialinksIsOpened($login,'um_liveinfos.players.1')){
			manialinksHide($login,'um_liveinfos.players.1');
		}
		...
		if($ppos>1 && $position['Prev2Login']!='' && isset($_players[$position['Prev2Login']])){
			if($action & 32){
				...
				manialinksShow($login,'um_liveinfos.players.2',$xml);
			}
		}elseif(manialinksIsOpened($login,'um_liveinfos.players.2')){
			manialinksHide($login,'um_liveinfos.players.2');
		}
		...
		if($ppos>0 && $position['PrevLogin']!='' && isset($_players[$position['PrevLogin']])){
			if($action & 4){
				...
				manialinksShow($login,'um_liveinfos.players.3',$xml);
			}
		}else{
			manialinksHide($login,'um_liveinfos.players.3');
		}
		...
		if($position['NextLogin']!='' && isset($_players[$position['NextLogin']])){
			if($action & 8){
				...
				manialinksShow($login,'um_liveinfos.players.5',$xml);
			}
		}elseif(manialinksIsOpened($login,'um_liveinfos.players.5')){
			manialinksHide($login,'um_liveinfos.players.5');
		}
		...
		if($position['Next2Login']!='' && isset($_players[$position['Next2Login']])){
			if($action & 64){
				...
				manialinksShow($login,'um_liveinfos.players.6',$xml);
			}
		}elseif(manialinksIsOpened($login,'um_liveinfos.players.6')){
			manialinksHide($login,'um_liveinfos.players.6');
		}
	...
	//manialinksHideHudPart('um_liveinfos','checkpoint_list',$login);
}
